...
class AnalyticsService {
  ...
  async getTopPlans(orgId, months) {
    ...
    // Group by plan type name, with variants nested inside
    const planMap = {};
  ...
  async getPaymentMethods(orgId) {
    ...
    return groups.map((g) => ({
      ...
      percentage:
        total > 0
          ? Math.round(((g._sum.amount || 0) / total) * 1000) / 10
          : 0,
    }));
  }
  ...
  async getTrainerPerformance(orgId) {
    ...
    const totalMap = Object.fromEntries(
      trainingGroups.map((g) => [g.trainerId, g]),
    );
    const activeMap = Object.fromEntries(
      activeGroups.map((g) => [g.trainerId, g._count.id]),
    );
    ...
    return trainers
      .map((t) => {
        ...
        return {
          ...
          avgPlanPrice:
            totalCount > 0
              ? Math.round((totalRevenue / totalCount) * 100) / 100
              : 0,
        };
      })
  ...
  async getDemographics(orgId) {
    ...
    const gender = Object.entries(genderMap).map(([label, count]) => ({
      ...
      percentage:
        members.length > 0
          ? Math.round((count / members.length) * 1000) / 10
          : 0,
    }));
    ...
    for (const m of members) {
      ...
      const age = Math.floor(
        (now - new Date(m.dateOfBirth)) / (365.25 * 24 * 3600 * 1000),
      );
